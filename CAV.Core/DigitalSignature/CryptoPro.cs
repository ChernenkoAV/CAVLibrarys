...
#pragma warning restore 612
        ...
        /// <summary>
        /// Дополнительная информация для упрощения создания Xml подписи по ГОСТ 34.10.
        /// </summary>
        private static class CPSignedXml
        {
            /// <summary>
        ...
        public static String ViewDSigSMEV(String Xml)
        {
            ...
            foreach (XmlElement Sign in xmlDocument.GetElementsByTagName("Signature", SignedXml.XmlDsigNamespaceUrl))
            {
                ...
                foreach (Reference r in signedXml.SignedInfo.References)
                {
                    ...
                    if (areasign.NamespaceURI.Contains("http://schemas.xmlsoap.org/soap/envelope/") & areasign.LocalName == "Body")
                    {
