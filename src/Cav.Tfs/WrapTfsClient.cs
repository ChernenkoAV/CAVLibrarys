...
namespace Cav.Tfs
{
    ...
    public class WrapTfs
    {
        ...
        private static Assembly tfsClientAssembly;
        private static Assembly tfsVersionControlCommonAssembly;
        private static Assembly tfsVersionControlClientAssembly;
        private static Assembly tfsVersionControlControlsCommonAssembly;
        private static Assembly tfsVersionControlControlsAssembly;
        private static Assembly tfsWorkItemTrackingClientAssembly;
        ...
        public WorkspaceInfo WorkspaceInfoGet(String localFileName) =>
            new WorkspaceInfo() { WSI = workstationGet().InvokeMethod("GetLocalWorkspaceInfo", localFileName) };
        ...
        {
            if (wsi is null)
                throw new ArgumentNullException(nameof(wsi));

            var servUri = (Uri)wsi.WSI.GetPropertyValue("ServerUri");
        ...
        public VersionControlServer VersionControlServerGet(Uri serverUri)
        {
            ...
            var vcsType = tfsVersionControlClientAssembly.
        ...
        {
            if (vcs is null)
                throw new ArgumentNullException(nameof(vcs));
            if (wsi is null)
                throw new ArgumentNullException(nameof(wsi));
        ...
        public QueryHistoryParameters QueryHistoryParametersCreate(String localFileName, VersionSpec vs)
        {
            if (string.IsNullOrWhiteSpace(localFileName))
                throw new ArgumentException($"'{nameof(localFileName)}' не может быть пустым или содержать только пробел.", nameof(localFileName));
        ...
        {
            if (vsc is null)
                throw new ArgumentNullException(nameof(vsc));
            if (qhp is null)
                throw new ArgumentNullException(nameof(qhp));
            ...
            Boolean workspaceLocationOnServer)
        {
            ...
            if (string.IsNullOrEmpty(workspaceName))
                throw new ArgumentException($"\"{nameof(workspaceName)}\" не может быть неопределенным или пустым.", nameof(workspaceName));
            if (string.IsNullOrEmpty(workspaceComment))
                throw new ArgumentException($"\"{nameof(workspaceComment)}\" не может быть неопределенным или пустым.", nameof(workspaceComment));
            ...
            var localion = workspaceLocationOnServer
                ? tfsVersionControlCommonAssembly.GetEnumValue("WorkspaceLocation", "Server")
                : tfsVersionControlCommonAssembly.GetEnumValue("WorkspaceLocation", "Local");
            ...
            string localPath)
        {
            ...
            if (string.IsNullOrEmpty(serverPath))
                throw new ArgumentException($"\"{nameof(serverPath)}\" не может быть неопределенным или пустым.", nameof(serverPath));
            if (string.IsNullOrEmpty(localPath))
                throw new ArgumentException($"\"{nameof(localPath)}\" не может быть неопределенным или пустым.", nameof(localPath));
        ...
        public void WorkspaceAddFile(Workspace ws, string localPathFile)
        {
            ...
            if (string.IsNullOrEmpty(localPathFile))
                throw new ArgumentException($"\"{nameof(localPathFile)}\" не может быть неопределенным или пустым.", nameof(localPathFile));
        ...
        public void WorkspaceCheckIn(Workspace ws, string commentOnCheckIn, params int[] numberTasks)
        {
            if (ws is null)
                throw new ArgumentNullException(nameof(ws));
            if (string.IsNullOrEmpty(commentOnCheckIn))
                throw new ArgumentException($"\"{nameof(commentOnCheckIn)}\" не может быть неопределенным или пустым.", nameof(commentOnCheckIn));
            ...
            if (numberTasks.Any())
            {
                ...
                var workItemStoreType = tfsWorkItemTrackingClientAssembly.
                ...
                var workItemCheckinInfoType = tfsVersionControlClientAssembly.
                ...
                var associatedWorkItemsArray = Array.CreateInstance(workItemCheckinInfoType, associatedWorkItems.Count);

                for (var i = 0; i < associatedWorkItems.Count; i++)
        ...
        public void WorkspaceUndo(Workspace ws)
        {
            ...
            var cnt = (int)pc.PC.GetPropertyValue("Length");
            if (cnt == 0)
                return;
        ...
        public bool WorkspaceLockFile(Workspace ws, string serverPathItem, LockLevel lockLevel)
        {
            if (ws is null)
                throw new ArgumentNullException(nameof(ws));
            if (string.IsNullOrEmpty(serverPathItem))
                throw new ArgumentException($"\"{nameof(serverPathItem)}\" не может быть неопределенным или пустым.", nameof(serverPathItem));
        ...
        public long WorkspaceGetLastItem(Workspace ws, String serverItemPath)
        {
            ...
            if (string.IsNullOrEmpty(serverItemPath))
                throw new ArgumentException($"\"{nameof(serverItemPath)}\" не может быть неопределенным или пустым.", nameof(serverItemPath));
            ...
            var getOptionsGetAll = (int)tfsVersionControlClientAssembly.GetEnumValue("GetOptions", "GetAll");
            var getOptionsOverwrit = (int)tfsVersionControlClientAssembly.GetEnumValue("GetOptions", "Overwrite");
        ...
        public Uri ShowTeamProjectPicker(IWin32Window parentWindow)
        {
            ...
            return dr != DialogResult.OK ? null : (Uri)tpp.GetPropertyValue("SelectedTeamProjectCollection").GetPropertyValue("Uri");
        }
        /// <summary>
        ...
        public String ShowDialogChooseServerFolder(IWin32Window parentWindow, VersionControlServer vcs, String initalPath)
        {
            ...
            if (vcs is null)
                throw new ArgumentNullException(nameof(vcs));
            if (string.IsNullOrEmpty(initalPath))
                throw new ArgumentException($"\"{nameof(initalPath)}\" не может быть неопределенным или пустым.", nameof(initalPath));
            ...
            return dr != DialogResult.OK ? null : (String)dcsf.GetPropertyValue("CurrentServerItem");
        }
        /// <summary>
        ...
        public ReadOnlyCollection<QueryItemNode> QueryItemsGet(Uri serverUri, String serverItemPath)
        {
            ...
            var projectName = projectNoServerPath.GetPropertyValue("Name") as String;
        ...
        public ReadOnlyCollection<WorkItem> WorkItemsFromQueryGet(Uri serverUri, QueryItemNode queryitem)
        {
            ...
            var query = wis.InvokeMethod("GetQueryDefinition", queryitem.QueryID.Value);

            if (query == null)
                throw new ArgumentOutOfRangeException("Не найден запрос с именем " + queryitem.Name);

            var queryText = query.GetPropertyValue("QueryText") as String;
